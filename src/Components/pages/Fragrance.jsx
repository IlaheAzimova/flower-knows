...
function FragranceProductCard({ product }) {
...
function Fragrance() {
    ...
    // Baza Fragrance məhsulları
    const baseCollectionProducts = useMemo(() => {
        if (!products || products.length === 0) return [];
        return products.filter((p) => {
            const cat = (p.category || '').toLowerCase();
            const title = (p.title || '').toLowerCase();
            return cat === 'fragrance' || title.includes('perfume');
        });
    }, [products]);
    ...
    const fragranceProducts = useMemo(() => {
        ...
        if (selectedCollection === 'KNIGHT UNICORN') {
            list = list.filter((p) => {
                const title = (p.title || '').toLowerCase();
                ...
                return colStr.includes('unicorn') || title.includes('unicorn');
            });
        } else if (selectedCollection === 'STRAWBERRY CUPID') {
            list = list.filter((p) => {
                const title = (p.title || '').toLowerCase();
                const colStr = Array.isArray(p.collections)
                    ? p.collections.join(' ').toLowerCase()
                ...
                return colStr.includes('strawberry') || colStr.includes('cupid') || title.includes('strawberry') || title.includes('cupid');
            });
        }
