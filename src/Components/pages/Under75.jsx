...
// Under $75 üçün uyğun olan əsas setlər və məhsulların ID-ləri
const UNDER_75_IDS = [15, 16, 22, 38];
...
function ProductCard({ product }) {
    ...
    const valueTag = product.value || (originalPrice && originalPrice > finalPrice ? `VALUE ${formatPrice(Math.round(originalPrice))}` : null);
    ...
    return (
        <div className="group flex flex-col">
            <div className="relative overflow-hidden bg-[#faf9f8] aspect-[4/5]">
                <Link to={`/products/${toSlug(product.title)}`}>
                    ...
                    <div className="absolute top-2 right-2 z-10 flex flex-col items-end gap-1">
                        {valueTag && (
                            <span className="bg-[#ea9393] text-white text-[10px] tracking-[0.5px] uppercase px-2 py-0.5 font-medium">
                                {valueTag}
                            </span>
                        )}
                        {!valueTag && discountPercent > 0 && (
                            <span className="bg-[#ea9393] text-white text-[10px] tracking-[0.5px] uppercase px-2 py-0.5 font-medium">
                                {discountPercent}% OFF
                            </span>
                        )}
            ...
            <div className="text-center pt-3.5 pb-2 flex-1 flex flex-col justify-between">
                <div>
                    <h3 className="font text-[14px] text-[#212326] tracking-[0.5px] mb-1 leading-snug line-clamp-1">
...
function Under75() {
    ...
    // ID ilə baza məhsullar
    const baseCollectionProducts = useMemo(() => {
        if (!products || products.length === 0) return [];
        return products.filter((p) => UNDER_75_IDS.includes(Number(p.id)));
    }, [products]);

    // Filtrlənmiş və sıralanmış məhsul siyahısı
    const under75Products = useMemo(() => {
        let list = [...baseCollectionProducts];

        // A. Üstdəki Kateqoriya Filtri
        if (selectedCategory !== 'ALL') {
            ...
            list = list.filter((p) => {
                ...
                if (cleanSelected === 'accessoriesandtools' || cleanSelected === 'accessoriestools') {
                    return cleanCat.includes('access') || cleanCat.includes('tool');
                }
                if (cleanSelected === 'allin') {
                    return cleanCat === 'allin' || cleanCat === 'all-in';
                }
        ...
        // B. Açılan Filter menyusundakı Product Type-lar
        ...
        return sortedList;
    }, [baseCollectionProducts, selectedCategory, selectedTypes, minPrice, maxPrice, sortBy]);
        ...
        <div className="w-full bg-white min-h-screen">
            ...
            <div className="md:w-[90%] mx-auto px-4 md:px-8 pb-16">
                ...
                <p className="dmsans text-[12px] text-[#777] my-4">
                    {under75Products.length} products
                </p>

                {/* KATEQORİYA DÜYMƏLƏRİ */}
