...
function KnightUnicornCard({ product }) {
...
function KnightUnicorn() {
    ...
    // 1. Knight Unicorn kolleksiyasına aid 15 məhsul
    ...
    // 2. Məhsul siyahısının filtrlənməsi
    const unicornProducts = useMemo(() => {
        let list = [...baseCollectionProducts];

        // A. Üstdəki Kateqoriya Tab-ı
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {unicornProducts.map((product) => (
                            <KnightUnicornCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
