...
function BunnyGardenCard({ product }) {
        ...
        <div className="group flex flex-col">
            <div className="relative overflow-hidden bg-[#faf9f8] aspect-[4/5]">
                <Link to={`/products/${toSlug(product.title)}`}>
                    ...
                    {discountPercent > 0 && (
                        <div className="absolute top-2 right-2 z-10 bg-[#e8989a] text-white text-[10px] tracking-[0.5px] uppercase px-2 py-0.5 font-medium">
                            {discountPercent}% OFF
                        </div>
                    )}
                </Link>
                ...
                <button
                    ...
                    className="hidden lg:block absolute bottom-0 left-0 right-0 bg-[#e8989a] text-white font uppercase text-[12px] tracking-[2.5px] py-3.5 translate-y-full group-hover:translate-y-0 transition-transform duration-300 hover:bg-[#b9788a] z-20 cursor-pointer"
                >
                    Choose Options
                </button>
                {/* MOBİL: Səbət ikonu */}
...
function BunnyGarden() {
    ...
    // Filtr üçün state-lər
    const [selectedTypes, setSelectedTypes] = useState([]);
    const [minPrice, setMinPrice] = useState(0);
    const [maxPrice, setMaxPrice] = useState(null);

    // 1. Kolleksiyaya aid olanlar
    const baseCollectionProducts = useMemo(() => {
        ...
        return products.filter((p) => {
            ...
            return (
                ...
                collection === 'bunny-garden' ||
                tags.includes('bunny')
            );
        });
    }, [products]);

    // 2. Filtrlənmiş və sıralanmış məhsul siyahısı
    const bunnyProducts = useMemo(() => {
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {bunnyProducts.map((product) => (
                            <BunnyGardenCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
