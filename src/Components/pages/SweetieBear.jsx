...
function SweetieBearCard({ product }) {
...
function SweetieBear() {
    ...
    // Sweetie Bear kolleksiyası məhsullarını filterləyirik
    const baseCollectionProducts = useMemo(() => {
        ...
        return products.filter((p) => {
            ...
            return (
                ...
                tags.includes('bear') ||
                [20, 22, 26, 31, 32, 45].includes(p.id)
            );
        });
    }, [products]);

    // Filtrlənmiş və sıralanmış məhsul siyahısı
    const bearProducts = useMemo(() => {
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {bearProducts.map((product) => (
                            <SweetieBearCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
