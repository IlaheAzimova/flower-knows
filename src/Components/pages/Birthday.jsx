...
// Şəkildə olan 4 məhsulun ID-ləri
const BIRTHDAY_SET_IDS = [16, 27, 28, 17];
        ...
        <div className="group flex flex-col">
            <div className="relative overflow-hidden bg-[#faf9f8] aspect-[4/5]">
                <Link to={`/products/${toSlug(product.title)}`}>
                    ...
                    {valueTag && (
                        <span className="absolute top-2 right-2 z-10 bg-[#ea9393] text-white text-[10px] tracking-[0.5px] uppercase px-2 py-0.5 font-medium">
                            {valueTag}
                        </span>
                    )}
                </Link>
...
function Birthday() {
    ...
    // Birthday baza məhsulları
    const baseCollectionProducts = useMemo(() => {
        if (!products || products.length === 0) return [];
        return products.filter((p) => BIRTHDAY_SET_IDS.includes(Number(p.id)));
    }, [products]);
        ...
        <div className="w-full bg-white min-h-screen">
            ...
            <div className="md:w-[90%] mx-auto px-4 md:px-8 pb-16">
                ...
                <p className="dmsans text-[12px] text-[#777] my-4">
                    {birthdayProducts.length} products
                </p>
