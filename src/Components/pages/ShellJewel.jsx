...
function ShellJewelCard({ product }) {
    ...
    const baseCollectionProducts = useMemo(() => {
        ...
        return products.filter((p) => {
            ...
            return (
                ...
                [18, 36, 37].includes(p.id)
            );
        });
    }, [products]);

    // Filtrlənmiş və sıralanmış məhsul siyahısı
    const shellProducts = useMemo(() => {
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {shellProducts.map((product) => (
                            <ShellJewelCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
