...
function StrawberryCupidCard({ product }) {
...
function StrawberryCupid() {
    ...
    // Strawberry Cupid kolleksiyası baza məhsulları
    const baseCollectionProducts = useMemo(() => {
        ...
        return products.filter((p) => {
            ...
            return (
                colStr.includes('cupid') ||
                colStr.includes('strawberry-cupid') ||
                title.includes('cupid') ||
                tags.includes('cupid') ||
                [16, 25, 27, 28].includes(Number(p.id))
            );
        });
    }, [products]);

    // Filtrlənmiş və sıralanmış məhsul siyahısı
    const cupidProducts = useMemo(() => {
    ...
    return (
        <div className="w-full bg-white min-h-screen">
            {/* HERO BANNER */}
            <div
                className="relative w-full h-[220px] sm:h-[300px] md:h-[400px] bg-[#f7e6e9] bg-cover bg-center flex items-center justify-end pr-8 md:pr-24"
                style={{
                    backgroundImage: `url('https://flowerknows.co/cdn/shop/files/20250821-174735.jpg?v=1755769966&width=3000')`
                }}
            >
            ...
            <div className="md:w-[90%] mx-auto px-4 md:px-8 pt-8 pb-16">
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {cupidProducts.map((product) => (
                            <StrawberryCupidCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
...
export default StrawberryCupid;