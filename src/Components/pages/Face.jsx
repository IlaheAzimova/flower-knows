...
// Face səhifəsində olan kolleksiya filtrləri
const FACE_COLLECTIONS = [
    'ALL',
    'KNIGHT UNICORN',
    'BUNNY GARDEN',
    'THE SWEETIE BEAR',
...
function FaceProductCard({ product }) {
...
function Face() {
    ...
    // Baza Face məhsulları
    const baseCollectionProducts = useMemo(() => {
        if (!products || products.length === 0) return [];
        return products.filter((p) => p.category?.toLowerCase() === 'face');
    }, [products]);
        ...
        <div className="w-full bg-white min-h-screen">
            ...
            <div className="md:w-[90%] mx-auto px-4 md:px-8 pt-8 pb-16">
                ...
                <p className="dmsans text-[12px] text-[#777] my-4">
                    {faceProducts.length} products
                </p>

                {/* KOLLEKSİYA DÜYMƏLƏRİ */}
                <div className="flex flex-wrap gap-2 mb-10">
                    {FACE_COLLECTIONS.map((col) => (
                        <button
                            key={col}
                            type="button"
                ...
                {loading ? (
                    ...
                    <div className={`grid ${getGridClass()} gap-x-4 md:gap-x-6 gap-y-10`}>
                        {faceProducts.map((product) => (
                            <FaceProductCard key={product.id} product={product} />
                        ))}
                    </div>
                )}
            </div>
